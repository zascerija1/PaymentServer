...
import ba.unsa.etf.si.payment.request.PasswordRequest;
...
public class PasswordRecoveryController {
    ...
    public Question getSecurityQuestion(@Valid @RequestBody RecoverRequest recoverRequest){
        ...
        //todo dodati QuestionResponse sa atributima id, title, description
        if(user==null)
    ...
    public PasswordResponse getNewPassword(@Valid @RequestBody PasswordRequest passwordRequest) {
        ApplicationUser user=applicationUserService.getUserByUsernameOrEmail(passwordRequest.getUsernameOrEmail());
        if(user==null)
            throw new ResourceNotFoundException("User with credentials" + passwordRequest.getUsernameOrEmail() + " does not exist");
        if(passwordRequest.getAnswer().equals(user.getAnswer().getText())){
